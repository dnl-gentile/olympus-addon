...
local MAX_SHOWN_PER_MINUTE = 60  -- flood guard over every channel together
...
local TIERS = {
	A = { level = 1, label = "CHAN_ALL",      slash = "/ol",  word = "all",      deny = "MEMBERS_ONLY",       color = { 0.90, 0.77, 0.36 } },
...
local seen = {}      -- "sender#id" -> time: every part is shown once
local buckets = {}   -- sender -> { tokens, t }
local recent = {}    -- times of the lines shown in the last minute
...
function Channels.Receive(dist, sender, text, now)
	...
	-- The server stamps the sender, so this key can't be forged.
	local key = sender .. "#" .. m.id
	...
	for i = #recent, 1, -1 do
		if now - recent[i] > 60 then table.remove(recent, i) end
	end
	if #recent >= MAX_SHOWN_PER_MINUTE then
